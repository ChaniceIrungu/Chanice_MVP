 ...
 */
const isFunc = (func) =>
  func && func.constructor && func.call && func.apply ? true : false;
...
const deleteFile = (file, callback) =>
  fs.unlink(file, (err) => (err ? callback(err) : callback()));
  ...
  fs.rename(src, dst, (err) =>
    !err
      ...
      : copyFile(src, dst, (err) =>
          err ? callback(err) : deleteFile(src, callback)
        )
  );
...
const parseFileName = (opts, fileName) => {
  ...
  let { name, extension } = parseFileNameExtension(
