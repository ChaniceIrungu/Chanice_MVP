...
function next() {
  ...
  dicer
    ...
    .on("finish", function () {
      assert(
        finishes++ === 0,
        makeMsg(v.what, "finish emitted multiple times")
      );
      ...
      if (v.events && v.events.indexOf("preamble") > -1) {
        ...
        if (fs.existsSync(fixtureBase + "/preamble.header")) {
          var prehead = JSON.parse(
            fs.readFileSync(fixtureBase + "/preamble.header", "binary")
          );
          if (!preamble) {
            preamble = {
              body: undefined,
        ...
        if (fs.existsSync(fixtureBase + "/preamble.error")) {
          var err = new Error(
            fs.readFileSync(fixtureBase + "/preamble.error", "binary")
          );
          if (!preamble) {
            preamble = {
              body: undefined,
        ...
        for (var i = 0, header, body; i < v.nparts; ++i) {
          ...
          if (fs.existsSync(fixtureBase + "/part" + (i + 1) + ".header")) {
            header = fs.readFileSync(
              fixtureBase + "/part" + (i + 1) + ".header",
              "binary"
            );
            header = JSON.parse(header);
          } else header = undefined;
          assert.deepEqual(
            state.parts[i].header,
            header,
            makeMsg(
              v.what,
              "Part #" +
